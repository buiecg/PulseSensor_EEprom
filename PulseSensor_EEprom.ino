...
int stay,  //for delay 31 BPM
    adr_EPR;//EEPROM address
/*
...
const int  BUTTON_PIN = 3;     // EEPROM condition button
...
void setup() {
  ...
  */
  pinMode(BUTTON_PIN, INPUT_PULLUP);
   ...
   if (buttonState == LOW) {
      attachInterrupt(digitalPinToInterrupt(3), wakeUpNow, RISING); // use interrupt 1 (pin 3) and run function
                                      // wakeUpNow(function at the end of sketch) when pin 3 gets HIGH 
      while (!Serial) {
      ; // wait for serial port to connect. Needed for native USB port only
      }
      Serial.println(F("CLEARDATA")); //for PLX-DAQ (EXCEL file)
      Serial.println(F("LABEL, Data, Curent_time, Curent_number, BPM"));
      for ( int index = 0 ; index < EEPROM.length(); index++) { // send data from EEPROM to Excel file
        byte val_EPR = EEPROM.read(index);
        Serial.println( (String) "DATA,DATE,TIME, " + index + "," + val_EPR + ",AUTOSCROLL_20" );
        delay(100);
      }
...
void loop() {
  ...
  */
  ...
  if (pulseSensor.sawStartOfBeat()) {
    ...
    if ((buttonState == LOW) && (adr_EPR < EEPROM.length())){//
      stay += 1;
    }
     if (stay >= 15){     // save in EEPROM ones in 16th BPM reads
        ...
        adr_EPR += 1;
        digitalWrite(PULSE_BLINK, bitRead(adr_EPR, 0));
      }
